...
def page_feature_analysis():
    ...
    # ── SHAP Plots ───────────────────────────────────────────────────
    # Only show SHAP plots generated by the CURRENT run. No fallback to
    # root-level images — those are stale and predate feature exclusions
    # (e.g. an old plot once surfaced ICULOS, which is no longer a feature).
    ...
    if shap_path.exists() or bar_path.exists():
